...
void residualForChemo(FEValues<dim>& fe_values, unsigned int DOF, FEFaceValues<dim>& fe_face_values, const typename DoFHandler<dim>::active_cell_iterator &cell, double dt, dealii::Table<1, Sacado::Fad::DFad<double> >& ULocal, dealii::Table<1, double>& ULocalConv, dealii::Table<1, Sacado::Fad::DFad<double> >& R, double currentTime, double totalTime) {
  ...
  for (unsigned int q=0; q<n_q_points; ++q) {
    c[q]=0.0; c_conv[q]=0.0; phi[q]=0.0; phi_conv[q]=0.0 ; 
  ...
  for (unsigned int i=0; i<dofs_per_cell; ++i) {
    ...
    for (unsigned int q=0; q<n_q_points; ++q) {
      ...
      Sacado::Fad::DFad<double>  newdFdC = (1-phi[q]*phi[q])*(phi[q]-lam*c[q]+lam*c[q]*phi[q]*phi[q]);
      dfdc=newdFdC;  	
      ...
      double angle = theta;
      //  angle = 0.125;     
      Sacado::Fad::DFad<double> W = W0*(1+ em*std::cos(mm*(angle-theta0))) ;
      Sacado::Fad::DFad<double> Wprime = -W0*em*mm*std::sin(mm*(angle-theta0)) ;		   
      Sacado::Fad::DFad<double> tau = tau0*(1+ em*std::cos(mm*(angle-theta0))) ;		   

      dealii::Table<1,Sacado::Fad::DFad<double> > BIGTERM(dim) ;
      BIGTERM[0]=W*W*phi_j[q][0] - W*Wprime*phi_j[q][1] ;
      BIGTERM[1]=W*W*phi_j[q][1] + W*Wprime*phi_j[q][0] ;
      
      
      if (ck==0) {
	R[i] +=  (1/dt)*fe_values.shape_value(i, q)*(c[q]-c_conv[q])*fe_values.JxW(q);  
	R[i] += - (0.5/dt)*fe_values.shape_value(i, q)*(phi[q] - phi_conv[q])*fe_values.JxW(q);
	...
	R[i] += -(1.0/tau)*fe_values.shape_value(i, q)*(dfdc)*fe_values.JxW(q);

	for (unsigned int j = 0; j < dim; j++) {
	  R[i]+= (1/tau)*fe_values.shape_grad(i, q)[j]*BIGTERM[j]*fe_values.JxW(q);
