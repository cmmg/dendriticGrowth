  ...
  for (unsigned int i=0; i<dofs_per_cell; ++i) {
	...
	JATF[1]=(1-ke*(DS/DL))*root2*(1+(1-ke)*c[q])*(1.0/dt)*(phi[q]-phi_conv[q])*(phi_j[q][1]/gradMAG);
      ...
      */
      ...
      Sacado::Fad::DFad<double> PP_phi = (15.0/8)*(phi[q]-(2.0/3.0)*phi[q]*phi[q]*phi[q] +(1.0/5.0)*phi[q]*phi[q]*phi[q]*phi[q]*phi[q]);
      Sacado::Fad::DFad<double> PP_phi_conv = (15.0/8)*(phi_conv[q]-(2.0/3.0)*phi_conv[q]*phi_conv[q]*phi_conv[q] +(1.0/5.0)*phi_conv[q]*phi_conv[q]*phi_conv[q]*phi_conv[q]*phi_conv[q]);
      Sacado::Fad::DFad<double> crossTerm = (0.5/dt)*(PP_phi-PP_phi_conv);  
